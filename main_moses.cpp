	...
	for(unsigned int S=0; S<steps; S++){
		...
		if (action == 0){
			...
			while (!STOP){
				...
				insertEvid(cluster,getTrainAcc(fold,FSID),getTestAcc(fold,FSID),sid,getModel(fold,FSID));
...
int main(int argc, char *argv[]){
	...
	for (unsigned int FOLD = 0; FOLD < numberOfFolds; FOLD++) {
		...
		for (unsigned int indiv = 0; indiv < populationSize; indiv++) {
			system(("rm -Rf "+string(GENE_TUPLET_OUTPUT_DIR)).c_str());
			createFoldSet(FOLD, sid);
			...
			insertEvid(cluster,getTrainAcc(FOLD,indiv),getTestAcc(FOLD,indiv),sid,getModel(FOLD,indiv));
